 ...
 */
 ...
 if (!function_exists('is_valid_email')) {
    ...
    function is_valid_email($email, $section_name) {
        $ci = & get_instance();
        $num_rows = 0;
        $data = array(
            'class_id' => $class_id,
...
if (!function_exists('email_validation')) {
    ...
    function email_validation($email) {
        $ci = & get_instance();
        $num_rows = 0;
        $user_array = array('admin', 'employee', 'parent', 'technical_support');
...
// class routine duplication on create
if (!function_exists('duplication_of_class_routine_specialist_on_create')) {

    function duplication_of_class_routine_specialist_on_create($data) {
        $ci = & get_instance();
        $num_rows = 0;
        $ci->db->where($data);
        $num_rows = $ci->db->get('class_routine_specialist')->num_rows();
        if ($num_rows == 0) {
            return 1;
        } else if ($num_rows > 1) {
            return 0;
...
// class routine duplication on edit
if (!function_exists('duplication_of_class_routine_specialist_on_edit')) {

    function duplication_of_class_routine_specialist_on_edit($data, $class_routine_specialist_id) {
        $ci = & get_instance();
        $num_rows = 0;
        $ci->db->where_not_in('class_routine_specialist_id', $class_routine_specialist_id);
        $ci->db->where($data);
        $num_rows = $ci->db->get('class_routine_specialist')->num_rows();
        if ($num_rows == 0) {
            return 1;
