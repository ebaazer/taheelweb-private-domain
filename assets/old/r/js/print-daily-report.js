(function(window, moment, Promise, $, Ractive) {
    ...
    var curDate = moment(window.reportDate, "YYYY-MM-DD")
    var lastDate = moment(window.endDate, "YYYY-MM-DD");

    var daysInMonth = 100;//(monthDate.daysInMonth());
    var days = [];
    var lookupDays = [];
    var dates = [];
    ...
    while(curDate < lastDate) {
        var day = curDate.format('dddd');
        if (day == 'Friday' || day == 'Saturday') {

        } else {
            days.push(moment(curDate).locale('ar').format('dddd'));  // your format
            dates.push(curDate.format('M-D'));
            lookupDays.push(curDate.format('YYYY-MM-DD'));
        }
    ...
    var formattedSteps = window.monthlySteps.reduce(function(val, cur) {
        ...
        stepRpt.status[formattedDate] = window.symbolsAbbr[evaluation] || evaluation;
        return val;
    }, []);
