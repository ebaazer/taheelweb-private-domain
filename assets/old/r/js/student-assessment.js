    ...
    var assessment = new Ractive({
        ...
        pickStudent: function (months) {
            var student = {
                ...
                dateOfBirth: date,
                dateOfBirthHijri: this.dateToHijri(date) || 0,
        ...
        summaryAssessment: function (plan) {
            ...
            /*
            Object.keys(summary).forEach(function(summ) {
                Object.keys(summary[summ].goals).forEach(function(goal) {
                    summary[summ].goals[goal].steps.sort(function(step1, step2) {
                        if (step1.startAgeInMonth === step2.startAgeInMonth) {
                            return step1.endAgeInMonth >= step2.endAgeInMonth ? 1 : -1;
                        }
                        return step1.startAgeInMonth > step2.startAgeInMonth ? 1 : -1;
                    });
                });
            });
            */
        ...
        savePlan: function () {
            ...
            /*
            var stepsMastered = assessment.mastered.map(function (step) {
                return {
                    step_id: step.id,
                    step_performance_id: step.step_performance,
                    assessment_id: _this.get('selected')
                };
            });
            */
            //console.log(stepsMastered);
            ...
            //Promise.all([assessmentPromise, planPromise]).then(function () {
            //   
            //})
