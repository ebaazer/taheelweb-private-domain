    ...
    $(function () {
        var homeWizard = new Ractive({
            ...
            placeStudentPlan: function (plan) {
                var _this = this;
                var url = BASE + "place_student_home_plan";
                return fetch(url, {
                    'method': 'POST',
                    credentials: "same-origin",
                    'headers': {
            ...
            refresh: function () {
                document.location.href = BASE + 'home_plan';
            },
            pushPlan: function () {
        ...
        if (window.auto) {
            ...
            console.log(entry);
            homeWizard.set('studentId', entry.student_id).then(function () {
                return homeWizard.fetchStudentPlans(entry.student_id)
            }).then(function () {
                return homeWizard.set('planId', entry.plan_id);
            }).then(function () {
                return homeWizard.set('monthId', entry.month_id);
            }).then(function () {
                return homeWizard.showWizard();
            });
        }
    });
