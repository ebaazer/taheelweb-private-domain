    ...
    $(function () {
        var planWizard = new Ractive({
            ...
            showWizard: function () {
                ...
                Promise.all([this.fetchStudentSteps(planId), this.fetchStudentInfo(studentId)]).then(function (results) {
                    ...
                    var term_name = planWizard.term_name;
                    var term_start_date = planWizard.term_start_date;
                    var term_end_date = planWizard.term_end_date;
            ...
            initWizard: function () {
                var ractive = this;
                var wizard = new KTWizard("monthly_plan_wizard", {
                    startStep: 1
                });

                $('#monthly_plan_wizard').find('[data-wizard-type="action-submit"]').on('click', function (e) {
                    e.preventDefault();
                    ractive.pushPlan();
                });
            },
            ...
            placeStudentPlan: function (plan) {
                ...
                return fetch(url, {
                    ...
                    //return results;
                    Swal.fire(lang_plan_saved, lang_monthly_plan, "success");
                    $('#monthly_plan_datatable').KTDatatable('reload');
                    $('#modal_ajax_xl').modal('hide');
                });
            },
            refresh: function () {
        ...
        if (window.auto) {
            ...
            planWizard.set('studentId', entry.student_id).then(function () {
                ...
                return planWizard.set('term_name', entry.term_name);
            }).then(function () {
                return planWizard.set('term_start_date', entry.term_start_date);
            }).then(function () {
                return planWizard.set('term_end_date', entry.term_end_date);
            }).then(function () {
