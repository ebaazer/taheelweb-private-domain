...
eventDrop: function (cxt) {
    console.log('🔄 eventDrop:', cxt);

    var rawEvent = _this.eventToRaw(cxt.event);

    // ✅ التأكد من أن `id` و `schedule_id` لا يكونان `undefined`
    rawEvent.id = cxt.event.id || cxt.event.extendedProps?.id || '';
    rawEvent.schedule_id = cxt.event.extendedProps?.schedule_id || '';

    // ❌ منع إرسال بيانات غير مكتملة
    if (!rawEvent.id || !rawEvent.schedule_id) {
        console.warn('⚠️ حدث بدون ID أو Schedule_ID، لن يتم تحديثه:', rawEvent);
        return;
    }

    console.log('✅ Updated Event:', rawEvent);

    _this.postEvent(rawEvent).then(function(response) {
        if (response.success) {
            console.log('✅ تم تحديث الحدث بنجاح.');
        } else {
            console.error('❌ فشل في تحديث الحدث:', response);
        }
    }).catch(function(error) {
        console.error('❌ خطأ أثناء تحديث الحدث:', error);
    });
},
...
eventToRaw: function (fcEvent) {
    ...
    return {
        id: fcEvent.id || fcEvent.extendedProps?.id || '',  // ✅ تأكد من الاحتفاظ بـ ID
...
rawToEvent: function (rawEvent) {
    ...
    return {
        id: rawEvent.id,  // ✅ تأكد من تخزين ID بشكل صحيح
        start: baseDate + " " + rawEvent.start_time,
        end: baseDate + " " + rawEvent.end_time,
        title: title,
        className: "fc-event-info",
        extendedProps: {
            schedule_id: rawEvent.schedule_id,  // ✅ تأكد من تخزين Schedule_ID
...
postEvent: function (rawEvent) {
    ...
    return fetch(url, {
        ...
        // ✅ تحديث `id` بعد الإضافة أو التعديل
        if (results.id) {
            rawEvent.id = results.id;
        }
        if (results.schedule_id) {
            rawEvent.schedule_id = results.schedule_id;
        }

        // ✅ إعادة تحميل الجدول لضمان حفظ `id`
        _this.fetchSchedules();
