    ...
    var specialistSchedule = new Ractive({
        ...
        newEventModal: function (eventEntry) {
            ...
            return new Promise(function (resolve, reject) {
                ...
                availStudents.then(function (results) {
                    ...
                    //console.log('busyStudents', busyStudents);
                    //console.log(results.busyQuery);
                    _this.set('busy', busyStudents);
                    ...
                    var studentsToShow = _this.get('STUDENTS').filter(function (student) {
                        return true;//modalStudents.indexOf(student.student_id) > -1;
                    });
        ...
        rawToEvent: function (rawEvent) {
            moment.updateLocale('en', {
                week: {
                    dow: 6  // تعيين يوم السبت كبداية للأسبوع
                }
            });

            // تحويل قيمة day_id إلى رقم
            var day = +rawEvent.day_id;
            // تعديل قيمة اليوم: إذا كان day هو 6 (سبت حسب البيانات) يصبح 0، وإلا نضيف 1
            var adjustedDay = (day + 1) % 7;

            // الآن startOf('week') سيعيد تاريخ السبت (لأننا قمنا بتحديث locale)،
            // وبإضافة adjustedDay نحصل على التاريخ الصحيح لباقي الأيام.
            var baseDate = moment().startOf('week').add(adjustedDay, 'days').format("YYYY-MM-DD");

            // باقي تحويل الحدث كما هو...
        ...
        fetchSchedules: function () {
            ...
            var employee_id = document.getElementById("employee_id").value;
            var class_id = document.getElementById("class_id").value;
            var url = BASE + "api_fetch_schedules/" + employee_id + '/' + class_id;
            return fetch(url, {
                method: 'GET',
                credentials: 'same-origin',
            }).then(function (response) {
        ...
        printSchedule: function (schedule) {
            ...
            var print = "<link href='../assets/fullcalendar/fullcalendar.bundle.rtl.css' rel='stylesheet' type='text/css'/>"
                    + element.innerHTML;
        ...
        renderCalendar: function (schedule) {
            ...
            var calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                editable: true, // تعطيل السحب والنقل
                selectable: canEditSchedule,
                selectOverlap: false,
                selectHelper: canEditSchedule,
                eventOverlap: false,
                contentHeight: 10,
                firstDay: 6, // تحديد أول يوم في الأسبوع
                eventStartEditable: true, // السماح نقل الأحداث
                eventDurationEditable: true, // السماح بتغيير المدة الزمنية
                columnHeaderText: function (date) {
                    return window.days[moment(date).format("ddd")];
                },
                eventRender: function (data) {
                    //console.log("Rendering event:", data.event);
                    var calendar = this;
