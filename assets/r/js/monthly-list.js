(function (window, Ractive, moment) {
    ...
    var plan = new Ractive({
        ...
        fetchMonthlyPlan: function (monthlyPlanId) {
            var _this = this;
            var url = BASE + "fetch_monthly_plan/" + monthlyPlanId;
            return fetch(url, {
                method: "GET",
                credentials: "same-origin"
            })
        ...
        updateAnalysisEntry: function (stepKeyPath, step) {
            ...
            if (analysis.length == 0) {
                ...
                var msg = document.getElementById('noAnalysis').value;
            ...
            var url = BASE + "post_monthly_analysis/" + planId;

            var monthlyPlanId = this.get('monthlyPlanId');
            var obj = {
                step: {
                    step_id: step.id,
        ...
        showAddPrivateAnalysisModal: function (step) {
            var newAnalysis = this.get('analysisLookup.' + step.id) || [];
        ...
        appendNewAnalysis: function () {
            this.push('analysisModal.newAnalysis', {
                analysis_name: ''
            });
        },
        savePrivateAnalysis: function (analysisModal) {
            var _this = this;
            var step = analysisModal.step;
            var analysis = analysisModal.newAnalysis;
            var url = BASE + "post_private_analysis";
            //console.log(step);
            var step_entry = {
                assessment_id: step.assessment_id,
                genre_id: step.genre_id,
            ...
            return fetch(url, {
                ...
                return response.json();
            }).then(function (newAnalysis) {
                newAnalysis.forEach(function (analysis) {
                    analysis.checked = true;
                });
                _this.set('analysisLookup.' + step.id, newAnalysis);

                var url = BASE + "post_monthly_analysis/" + planId;

                var monthlyPlanId = _this.get('monthlyPlanId');
                var obj = {
                    step: {
                        step_id: step.id,
                        active: '1',
                    ...
                    analysis: newAnalysis.map(function (entry) {
                        return {
                            monthly_plan_id: monthlyPlanId,
                            step_id: entry.step_id,
                            analysis_id: entry.analysis_id,
    ...
    var monthlyPlanId = document.getElementById("monthlyPlanId").value;
    ...
    var promises = [
        ...
        plan.fetchMonthlyPlan(monthlyPlanId)
    ]
    ...
    Promise.all(promises).then(function (results) {
        ...
        var analysisLookup = planData.analysis.reduce(function (val, cur) {
            ...
            if (!rawAnalysis) {
                //console.log(cur.step_id, "has no analysis", cur.analysis_id);
                return val;
            }
            cur.analysis_name = rawAnalysis.analysis_name;
            //console.log(cur.analysis_id, cur.analysis_name);
